...
class main_window(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        splitter = wx.SplitterWindow(self, style=wx.SP_3DBORDER)
    ...
    def project_save(self):
        ...
        try:
            ...
            MsgDlg(
                self, 'There was an error saving the new project file.', 'Error!', wx.OK)
    ...
    def OnProjectOpen(self, event):
        ...
        if open_it:
            dlg = wx.FileDialog(
                self, 'Choose a project to open', '.', '', '*.wxp', wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)  # wx.OPEN to wx.FD_OPEN | wx.FD_FILE_MUST_EXIST
    ...
    def OnProjectNew(self, event):
        ...
        if open_it:
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                dlg.Destroy()
                dlg = wx.FileDialog(
                    self, 'Place to store new project.', '.', '', '*.wxp',
                    wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)  # change from wx.SAVE
                if dlg.ShowModal() == wx.ID_OK:
                    try:
        ...
        if self.root:
            if self.activeitem != self.root:
                if self.editor.IsModified():  # Save modified file before
                    result = MsgDlg(
                        self, 'The edited file has changed.  Save it?')
                    if result == wx.ID_YES:
    ...
    def OnFileAdd(self, event):
        ...
        dlg = wx.FileDialog(self, 'Choose a file to add.',
                            '.', '', '*.*', wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)  # wx.OPEN to wx.FD_OPEN | wx.FD_FILE_MUST_EXIST
